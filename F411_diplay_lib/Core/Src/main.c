...
#define B_PARAM_THERMISTOR 3950
#define R10K 9980
#define T0  298.15f
...
struct GAS {
	uint16_t actual, last, BIT12, temp_BIT12;
	float real_temp, Vout_Temp;
} ch4;
...
void getTemp(uint16_t bit) {
	...
	uint16_t Vout = bit * (V_SUPPLY / 4095);

	Vout_Temp = (Vout * R10K) / (V_SUPPLY - Vout);

	ch4.real_temp = ((T0 * B_PARAM_THERMISTOR)
			/ (T0 * (long) (Vout_Temp / R10K) + B_PARAM_THERMISTOR)) - 273.15;
}
/* USER CODE END 4 */
...
//		sprintf((char*) data, "-%i ", ch4.temp_BIT12);
//		CDC_Transmit_FS(data, strlen((char*) data));
//		sprintf((char*) data, "%.2f ", Celsius_temp);
//		CDC_Transmit_FS(data, strlen((char*) data));
//		ch4.temp_BIT12 = ch4.temp_BIT12 * (V_SUPPLY / 4095);
//		ch4.Vout_Temp = (ch4.temp_BIT12 * R10K) / (V_SUPPLY - ch4.temp_BIT12);
//		ch4.real_temp = ((T0 * B_PARAM_THERMISTOR)
//				/ (T0 * (long) (ch4.Vout_Temp / R10K) + B_PARAM_THERMISTOR))
//				- 273.15;
//
//		sConfig.Channel = ADC_CHANNEL_6;
//		sConfig.Rank = 2;
...
//		getTemp(ch4.temp_BIT12);

//		Vout = ch4.BIT12 * (V_SUPPLY / 4095);
//
//		Vout_Temp = (Vout * R10K) / (V_SUPPLY - Vout);
//
//		ch4.real_temp = ((T0 * B_PARAM_THERMISTOR)
...
//		sprintf((char*) data, " %i %.2f %i\n", ch4.BIT12, Celsius_temp,
//				ch4.temp_BIT12);
//		CDC_Transmit_FS(data, strlen((char*) data));

//		sprintf((char*) data, " %i %.2f %i", ch4.BIT12, ch4.real_temp,
//				ch4.temp_BIT12);
//		CDC_Transmit_FS(data, strlen((char*) data));
